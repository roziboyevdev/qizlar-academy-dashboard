...
const DauMauStatisticsTable: React.FC = () => {
  ...
  const fetchData = async (): Promise<void> => {
    ...
    try {
      const response = await http<ApiResponse>(`https://back.qizlarakademiyasi.uz/api/v1/statistics/dau/mau/${selectedMonth}/${amount}`, {
        headers: {
  ...
  const downloadExcel = async (): Promise<void> => {
    ...
    try {
      const response = await http(`https://back.qizlarakademiyasi.uz/api/v1/statistics/dau/mau/file/${selectedMonth}/${amount}`, {
        responseType: 'blob',
        headers: {
          'Content-Type': 'application/json',
        },
