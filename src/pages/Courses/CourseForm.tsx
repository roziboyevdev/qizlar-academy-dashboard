...
const typeData = [{ type: "SUBJECT", name: "Fan" }, { type: "PROFFESION", name: "Kasb" }]
const levelData = [
  { type: "EASY", name: "Oson" },
...
const courseSchema = z.object({
  ...
  seoTitle:z.string().min(50,{message:'title kamida 50 ta harifdan iborat text kirgazing'}).max(65,{message:'title  maximal 65 ta harifdan iborat text kirgazing'}),
  seoDescription:z.string().min(120,{message:'description uchun kamida 120 ta harifdan iforat text kirgazing'}).max(320,{message:'description maximal 320 ta harifdan iforat text kirgazing'}),
  seoKeywords: z.string().min(8, { message: "Kamida 8 ta kalit so'z kiriting" }),
    // .array(z.string())
});
...
export default function CourseForm({ course, setSheetOpen }: IProps) {
  ...
  async function onSubmit(formValues: courseFormSchema) {
    ...
    const seo = {
      ...
      // image:withBanner.icon
    }
    // delete withBanner?.seoDescription
    // delete withBanner?.seoTitle
    // delete withBanner?.seoKeywords
console.log({ ...withBanner, seo });

    // if (course) {
    //   triggerCourseEdit({ ...withBanner, seo });
    // } else {
    //   triggerCourseCreate({ ...withBanner, seo });
    // }
  }
    ...
    <Form {...form}>
      ...
      >
        <div className="flex gap-4 flex-col my-4">
          ...
          <hr />
          <TextField name="seoTitle" label="Seo title - (50 - 65)"  placeholder="Bepul Targeting Kursi - Ustoz Aida Bilan O'rganing | Tezroq Boshlang" required />
          <TextAreaField name="seoDescription" label="Seo description (120 - 320)"  placeholder="Targeting kursini ustoz aida o'rganing . Kursimiz davomida  Reklama strategiyalarini tuzish, mijozlarni aniq aniqlash, va onlayn savdoni oshirishni o'rganib, o'z biznesingiz yoki brendingiz uchun muvaffaqiyatli targeting yaratishga erishing. Bepul darslar bilan boshlang!" required />
          <KeywordField name="seoKeywords" label="Seo seoKeywords"  placeholder="Target reklama, pul ishlash, targeting kurslar, bepul kurslar" required />
        </div>
        {course ? (
          <LoadingButton isLoading={isFileUpload || isCourseEditPending}>
            Tahrirlash
          </LoadingButton>
        ) : (
          <LoadingButton isLoading={isFileUpload || isCourseCreatePending}>
            Saqlash
          </LoadingButton>
        )}
      </form>
